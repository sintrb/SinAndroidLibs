...
public class TemplateEngine {
    ...
    public String evalString(Object model, String tmpl) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        ...
        while (m.find()) {
            ...
            Object v = expressionEngine.eval(model, exp);
            tmpl = tmpl.replace(TMPLESTART + exp + TMPLEEND, v == null ? "" : "" + v);
        }
        return tmpl;
    }

    public Object evalObject(Object model, String tmpl) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        Matcher m = tmplPattern.matcher(tmpl);
        if (m.find()) {
            String exp = m.group(1).trim();
            return expressionEngine.eval(model, exp);
        } else {
            return null;
        }
    }
